...
`2`的吧。所以这时候 while 循环终止是正确的，直接返回 `-1` 即可。
...
比如说给你有序数组 `nums = [1,2,2,2,3]，target = 2`，此算法返回的索引是 `2`\
1，没错。但是如果我想得到 target 的左侧边界，即索引 1\
2，或者我想得到 target 的右侧边界，即索引 `3`\
3，这样的话此算法是无法处理的。
...
3.  为什么没有返回 `−1`\
    `−1` 的操作？如果 `nums` 中不存在 `target` 这个值，怎么办？
...
<a href="https://leetcode-cn.com/problems/find-first-and-last-position-of-element-in-sorted-array/solution/er-fen-cha-zhao-suan-fa-xi-jie-xiang-jie-by-labula/" target="_blank" rel="noopener">出处</a>
