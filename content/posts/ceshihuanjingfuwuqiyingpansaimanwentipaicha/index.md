...
楼主首先想到的是可以看到，Linux 系统中占用数据盘最大的文件，常情况下，最有可能找出占用磁盘空间文件或文件夹的地方，主要是 `/tmp or /var or /home or /`。目前没有单个命令来完成查找的工作，通常可以使用一些命令的组合来帮助您找出磁盘上比较占用空间的文件或者文件夹。主要用到下面的三个命令：
